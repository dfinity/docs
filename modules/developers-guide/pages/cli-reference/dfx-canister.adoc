...
|<<dfx canister send,`+send+`>> |Send a signed `+message.json+` to create neurons.

|<<dfx canister send,`+sign+`>> |Create a signed `+message.json+` file to customize the process of creating neurons.
...
When creating your neuron stake, use the `+dfx canister send+` command with the `+dfx canister sign+` command when you want to separate these steps, rather than using the single `+dfx canister call+` command. Using separate calls can add security to the transaction.  
...
When creating your neuron stake, use the `+dfx canister sign+` command with the `+dfx canister send+` command when you want to separate these steps, rather than using the single `+dfx canister call+` command. Using separate calls can add security to the transaction.
...
Use the `+dfx canister sign+` command to create a signed `method.json` file using hte pricipal ID associated with the identitiy you created using the Privacy Enhanced Mail (PEM) file by running a command similar to the following:

`+dfx canister --network=ic sign --expire-after=1h rno2w-sqaaa-aaaaa-aaacq-cai create_neurons ‘(“PUBLIC_KEY”)’+`

This command illustrates how to creates a `+message.json+` file to create neurons that is signed using your principal identifier as the message sender and with an expiration window that ends in one hour.
