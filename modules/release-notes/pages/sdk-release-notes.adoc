...
* A fix to `+dfx upgrade+` that prevents a `+main+` thread panicked when `+Option::unwrap()+` is called on a `+None+` value. 
* A fix to the name of the language service command to restore its compatibility with IDEs. 

=== Candid

* Candid has been updated to support more Rust built-in types and with support for reference types.
