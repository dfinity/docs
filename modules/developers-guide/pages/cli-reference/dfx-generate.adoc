...
Currently, `+dfx generate+` support four languages: Motoko, Candid, JavaScript, TypeScript.
...
|`+canister_name+` |Specifies the name of the canister you want to generate type declarations.
...
If you don't specify this argument, all canisters declared in `+dfx.json+` will get types declarations generated.

|===

== Configuration

The behavior of `+dfx generate+` is controlled by `+dfx.json+` configuration file.
Under `+dfx.json+` → `+canisters+` → `+<canister_name>+`, you can add a `+declarations+` section.
In which, you can specify following fields:
...
Since there is only one canister in `+dfx.json+`, you can call `+dfx generate+` without argument.
