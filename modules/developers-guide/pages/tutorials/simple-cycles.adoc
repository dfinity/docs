...
* The program uses an `+actor class+` instead of a single actor so that it can have multiple actor instances.
...
This is important because you must be the owner of the wallet canister to perform certain tasks such as send cycles or grant other custodian identities permission to send cycles. 
