...
- An update to the `+dfx ping+` command enables you to specify `+network+` and a network name to check the status of a network connection.
- An update to authentication supports enables `+dfx+` to use the browser's `+localStorage+` for the user's public and private keys if cookies are not enabled.
- Motoko programming guidelines are now available as part of the programming language guide on the link:sdk.dfinity.org[SDK website].
