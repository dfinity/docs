...
link:../quickstart/quickstart-intro{outfilesuffix}[クイックスタート]で説明したように、ターミナルシェルでコマンドを実行することで、DFINITY Canister スマートコントラクトSDKパッケージの最新バージョンをダウンロードしてインストールすることができます。
...
例えば、`~/.cache/dfinity/versions/0.8.3` ディレクトリの内容をリストアップすると、以下のような主要コンポーネントが表示されます。
...
For example, if you list the contents of the `~/.cache/dfinity/versions/0.8.3` directory you would see the following core components:
