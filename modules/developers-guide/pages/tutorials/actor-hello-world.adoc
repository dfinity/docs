...
+
For example, the default canister name is `+actor_hello+` and the default path to the main program files is `+src/actor_hello/main.mo+`.
+
...
. Remove all pf the `+actor_hello_assets+` configuration settings from the file.
+
...
== Build the program using a locally-defined identifier
...
Before you can deploy this project, you need to connect to the {IC} network either running locally in your development environment or running remotely on a sub-network that you can access and generate a unique canister identifier to replace your locally-defined identifier.
...
----
+
This command create a `+canister_manifest.json+` file in the canister directory.
+
For example:
+
[source,json]
----
{
  "canisters": {
    "actor_hello": {
      "candid_path": "/Users/lisagunn/IC-Examples/actor_hello/canisters/actor_hello/actor_hello.did",
      "canister_id": "ic:03000000000000000000000000000000000179",
      "timestamp": "Fri, 12 Jun 2020 21:39:17 +0000",
      "wasm_path": "/Users/lisagunn/IC-Examples/actor_hello/canisters/actor_hello/actor_hello.wasm"
    }
  }
}
----
