...
When you create a new project, your project `+src+` directory includes a template `+main.rs+` file with the "Hello, World!" dapp.

To modify the template source code:

. Open the template `+src/main.rs+` file in a text editor and delete the existing content.
. Write the dapp you want to deploy on the {IC}.
+
As you write your dapp, keep in mind that there are two types of calls—update calls and query calls—and that update functions use asynchronous messaging.
...
* Connect to the {IC} network either running locally in your development environment or running remotely (on-chain) on a subnet that you can access.
...
dfx build
----
. Deploy the dapp on the local network by running the following command:
+
[source,bash]
----
...
To illustrate how to migrate dapps written in C to run on the {IC}, let’s look at the simple `+reverse.c+` dapp in the link:https://github.com/dfinity/examples/tree/master/c[examples] repository. 
The `+reverse.c+` dapp contains one function—named `+go+`—that reverses a string in place.

=== Set up the development environment

To compile the `+reverse.c+` dapp into WebAssembly, you need to have the `+clang+` compiler and standard libraries installed. 
...
=== Compile the dapp into WebAssembly
...
. Compile the dapp by running the following clang command:
+
[source,bash]
----
...
. Open a terminal in the `+build+` directory you created for the `+reverse.c+` dapp source
...
dfx canister create --all
----
. Deploy the default dapp on the local network by running the following command:
+
[source,bash]
----
dfx canister install --all
