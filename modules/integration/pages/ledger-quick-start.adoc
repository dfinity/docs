...
These command-line options to attach an interactive terminal session and remove the container are primarily for testing purposes. 
To run the image in a production environment, you might instead start the image using the `+--detach+` option to run the container in the background and, optionally, specify the `+--volume+` for storing blocks.

For more information about Docker command-line options, see the Docker documentation.
