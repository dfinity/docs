...
+
For example, click *Shell*, select *New Window*, then run the following command in the new terminal if your `+hello_location+` project is in the `+ic-projects+` working folder:
+
[source,bash]
----
...
[source,bash]
----
dfx canister call hello_location location '("San Francisco and Paris")'
----
The command displays output similar to the following:
+
....
...
....
+ 
Because your program only accepts a single string argument, specifying multiple strings returns a `+mismatch+` error.
+
