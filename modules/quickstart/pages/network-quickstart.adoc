...
[[net-quick-start]]
This _Quick Start_ scenario assumes that you are installing the {sdk-short-name} for the first time and deploying the default project on the {IC} blockchain.
...
[[ping-the-network]]
== Check the connection to the {IC} blockchain

Before you can deploy your first project on the {IC} blockchain, you need to connect to the network gateway running remotely on a subnet that you can access.
There is a reserved network alias that you can use to access the {IC} blockchain. The network alias is a system setting that's defined internally, so there's nothing you need to configure in your projects by default.

To check your connection to the {IC} blockchain:
...
dfx ping ic
----
. Verify that the `+dfx ping ic+` command returns information about the {IC} blockchain.
+
...
To deploy your first application on the {IC} blockchain:
...
This option is required to install on the {IC} blockchain.
+
...
* Using the `+--network ic+` option indicates that the canister you want to call is deployed on the `+ic+`. The `+ic+` network alias is an internally-reserved alias for accessing the {IC} blockchain.
