...
To get started, let's build and deploy a simple Hello application that has just one function—called `+greet+`. The `+greet+` function accepts one text argument and returns the result with a greeting similar to _Hello, everyone!_ in a terminal if you run the application using the command-line or in a pop-up window if you access the application in a browser.
