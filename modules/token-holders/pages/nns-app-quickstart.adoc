...
. Click **Voting**.
+
You can click any of the proposals listed to view information about the proposal, includinga brief description of the proposal, a link for viewing additional information about the proposal, the number of votes that were cast to adopt or reject the proposal, and the votes cast by your neurons.
...
There are now application subnets available for general use by the developer community. 
However, there is currently an issue in the Network Nervous System (NNS) application workflow that causes the creation of a new canister to fail and results in a fractional unit (e8s) loss.

You can deploy new canisters on the {IC} by using the {sdk-short-name} command-line interface (`+dfx+`) as described in link:../quickstart/network-quickstart{outfilesuffix}[Network deployment].
