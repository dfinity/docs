...
```bnf
<shared-pat> ::=        function sort and caller pattern
  shared query? <pat>?
```
...
| | | |
| `Blob` | `size` | `Nat` | length in bytes
| `Blob` | `bytes` | `{ next: () -> Word8? }` | byte iterator, first to last
| | | |
| `[var? T]` | `get` | `Nat -> [mut? T]` | indexed read
| `[var T]` | `set` | `(Nat, T) -> ()` | indexed write
| `[var? T]` | `keys` | `{ next: () -> Nat? }` | index iterator, in ascending order on Nat
| `[var? T]` | `vals` | `{ next: () -> T? }` | value iterator, in ascending index order.
|===

The projection `<exp> . <id>` evaluates `exp` to a result `r`. If `r` is `trap`, then the result is `trap`. Otherwise, `r` must be an
a value of type `U` and the result of the projection is a value of type `T`
whose semantics is given by the description in the table.

(NB: The `chars`, `bytes, `keys` and `vals` produce stateful *iterator objects* than can be
...
(NB: the exhaustiveness side condition on `shared` function expressions ensures that argument pattern matching cannot fail (see <<exp-func>>).)
...
* pattern `<pat>` has record type `{ caller : Principal }`;
...
(NB: Although general purpose, for expressions may, in particular, be used to iterate over iterator objects produced by special member access (see <<exp-dot>>).
...
The expression `continue <id>` is equivalent to `break <id_continue>`, where
 `<id_continue>` is implicitly declared around the bodies of `<id>`-labelled looping constructs (see Section <<exp-labeled-loops>>).
