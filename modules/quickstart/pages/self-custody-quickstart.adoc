...
+
....
dfx canister --network https://ic0.app governance manage_neuron "( record {id = opt record { id = <neuron_id>; command = opt variant { Configure = record { operation = opt variant {AddHotKey = record { new_hot_key = principal <new_hot_key_principal_id> }}}}}}"
....
+
