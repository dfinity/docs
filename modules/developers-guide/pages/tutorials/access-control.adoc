...
* Users who are assigned an `+admin+` or `+authorized+` role see a greeting that displays `+Would you like to play a game?+`.
...
....
. Assign this principal the `+admin+` role by running the following command using Candid syntax:
+
[source,bash]
----
