...
NOTE: There are now application subnets available for general use by the developer community. 
At this time, however, you can only deploy new canisters using the {sdk-short-name} command-line interface (`+dfx+`) as described in this tutorial. If you attempt to deploy a new canister using the Network Nervous System (NNS) application, the operation will fail and result in a fractional unit (e8s) loss.
