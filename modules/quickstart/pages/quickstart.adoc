...
+
Currently, the {sdk-short-name} only runs on computers with a macOS and Linux operating system.
...
+
You perform the remaining project-related steps in this terminal.
...
If you are connecting to an external {IC} network, you will need to use the `+--network+` option to deploy to that network.
...
For example, if you are connecting to the URL specified by the network alias `+tungsten+` you would run a command similar the following:
+
[source,bash]
----
dfx deploy --network tungsten
----
+
