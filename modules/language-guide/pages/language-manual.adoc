...
Operations on text values include concatenation (`_ # _`) and sequential iteration over characters via `t.chars` as in `for (c in t.chars()) { ... c ... }`.
...
shared functions and used for simple authentication. Although opaque, principals may be converted to binary `Blob`s for more efficient hashing and other applications, see module `Principal` from the standard library.
