...
You can access a preliminary version of the `+icx-nns+` command-line tool by downloading a release from th link:https://github.com/dfinity/icx-nns/releases[icx-nns] repository. For information about how to download `+icx-nns+` command-line tool and how to use the submit proposals, see https://www.internetcomputer.org/governance/submit-proposal[How to submit proposals to the Network Nervous System].
