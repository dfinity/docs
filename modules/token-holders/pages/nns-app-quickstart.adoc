...
For example, proposals might be submitted and require a vite when you are unavailable or propose changes that you lack the expertise to evaluate. 
...
For information about maturity and spawning new neurons, see the following articles:
...
. Verify your identification number, then click **Login** to authenticate using the device and authentication method you have registered.
. Click **Proceed** to access to the Network Nervous System (NNS) application. 
. Click **Neurons**, then click **Spawn Neuron**.
+
