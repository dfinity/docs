...
[source,bash]
----
"main": "src/calculator/calc_main.mo",
----
+
...
+
Because the cell variable in this program is an integer, you cam also call its functions and specifying negatives input values.
For example, you might run the following command:
+
[source,bash]
----
dfx canister call calculator mul '(-4)'
----
+
which returns:
+
