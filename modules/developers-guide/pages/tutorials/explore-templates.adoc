...
プレビューとして、以下のダイアグラムは、あなたのコンピュータ上でローカルに {IC} を動かす開発のワークフローを説明しています。
...
デフォルトでプロジェクトに含まれているファイルやフォルダを詳しく見るために、動かすための新しいプロジェクトを作っていきましょう。
...
----
+
`+dfx new explore_hello+` コマンドは、新しいプロジェクト名直下のデフォルトプロジェクト構造と新しい Git レポジトリを含めた `+explore_hello+` プロジェクトを作成します。もし `+node.js+` をローカルにインストールしていれば、新規のプロジェクトを作成することでテンプレートフロントエンドコードと依存ライブラリも追加されます。
+
...
+
--
* レポジトリのプロジェクトを説明する `README` ファイル
* プロジェクトのオプションを設定する `+dfx.json+` 環境設定ファイル
* Dapp に必要なすべてのソースファイルを持つ `+src+` ディレクトリ
--
+
デフォルトの `+src+` ディレクトリは、中核のプログラミングロジックを修正または取り替えるためのテンプレート `+main.mo+` ファイルを含みます。
+
...
デフォルトで、新しいプロジェクトを作成すると、プロジェクトディレクトリにいくつかのテンプレートファイルが追加されます。プロジェクトの環境設定をカスタマイズする、もしくは開発 Cycle を加速するために自身のコードを取り込むのに、これらのテンプレートファイルを編集することができます。
...
+
--
* `+settings+` セクションでは、`+explore_hello+` プロジェクトは `+explore_hello+` です、というような、WebAssembly モジュールの名前を指定します。
...
新しいプロジェクトは、常にテンプレート `main.mo` ソースコードファイルを含んでいます。開発 Cycle を加速する自身のコードを埋め込むために、このファイルを編集することができます。
...
----
+
プラットフォームやローカルセキュリティ設定によっては、警告の表示を見るかもしれません。もしネットワーク接続の許可もしくは拒否の選択を促されているなら、*Allow* をクリックしましょう。
+
...
ローカル Canister 実行環境に接続した後に、あなたのプロジェクトをユニークなネットワーク固有の *canister Identifiers* を生み出すネットワークとして登録できます。
...
ローカルネットワークとして、Canister Identifiers を登録する:

. プロジェクトディレクトリに位置することを確認します、必要であれば移動します。
. 以下のコマンドを走らせて、プロジェクトの Canister をユニークな Canister Identifiers として登録します。
+
[source,bash]
----
...
. ローカルコンピューターでターミナルシェルを開いていなければ開きます、また `+explore_hello+` プロジェクトディレクトリに移動します。
