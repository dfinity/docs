...
==== 3.4.2  Open a browser and see dapp

To see your dapp running locally in the browser you need your canister id from step 3.2.2 when you deployed to your local server.

Navigating to +http://localhost:8080/?canisterId=$canisterID+ where `canisterId` is the id displays a simple HTML page with a sample asset image file, an input field, and a button.
...
==== 4.2.1 Check your cycles balance
...
In the example above, we created a `hello` dapp that has two canisters: 
...
=== 4.5 Seeing and testing your dapp live on-chain
