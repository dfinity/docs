...
+
To work with this release, you should delete any existing canisters, then regenerate new canister identifiers using the `+dfx canister create+` command and redeploy with the `+dfx canister install+` command to begin using the new text-based identifiers.
