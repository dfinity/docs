...
=== ICPT-specific Rosetta API details
...
==== What is the `signature_type` / `curve_type` for ED25519?

- `signature_type` is `"ed25519"`
- `curve_type` is `"edwards25519"`

==== What kinds of transactions may appear in a block, and what do they mean?

- Each block as queried from the `[/block](https://www.rosetta-api.org/docs/BlockApi.html#block)` endpoint contains exactly one transaction.
- Transfer
...
- Fees do not apply to Mint/Burn operations.
...
==== How to send Mint/Burn transactions?
