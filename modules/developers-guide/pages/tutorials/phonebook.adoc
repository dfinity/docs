...
* You have stopped all processes related to the local canister execution environment provided by `dfx`.
...
For development purposes `dfx` provides a local canister execution environment. This requires a `+dfx.json+` file, so you should be sure you are in your project's root directory. For this tutorial, you should have two separate terminal shells, so that you can start and see the output of the processes related to the local canister execution environment in one terminal and manage your project in another.
...
After you finish experimenting with your dapp, you can stop the local execution environment so that it does not continue running in the background.
