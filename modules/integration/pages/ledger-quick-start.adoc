...
An account belongs to (and is controlled by) an account owner (which must be an IC principal); no account can be owned by two or more IC principals (i.e. no “joint accounts”).

An account owner may control more than one account -- each account therefore corresponds to  a pair (account_owner, sub_account); here, sub-account is an optional bitstring which helps distinguish between the different sub-accounts of the same owner.
...
The entire chain is regularly authenticated by signing the latest tip of the chain. The signature used to authenticate a transaction can be verified by any third party who has access to the root public key of the Internet Computer.
...
This document outlines how to integrate with the ledger canister using the https://www.rosetta-api.org/[Rosetta application programming interface]. This is a https://www.rosetta-api.org/docs/welcome.html[well-documented and open standard] designed to support multiple block chain data formats and structured communication for exchange workflows.
...
The following steps summarize the basic operational workflow for transferring ICP tokens if using a Rosetta node to communicate with the {IC} ledger canister.  The description concerns an {IC} principal who authenticates towards the {IC} using a signing key stored in a wallet.
...
* Account identifiers are 32 bytes and, roughly speaking, are the hash of a public key (together with some additional sub-account specifier).
