...
|<<dfx wallet send,`+send+`>> |Sends a specified amount of cyles from the selected identity's cycles wallet to another cycles wallet using the destination wallet canister ID. 
...
Use the `+dfx wallet add-controller+` command to add controllers to the selected identity's cycles wallet using their principals. 
...
|`+<controller>+` | Specify the principal of the identity you would like to add as a controller to the selected identity's cycles wallet.
|===
...
* `+Name+` if the cycle wallet's name is set. 
...
Use the `+dfx wallet authorize+` command to authorize a cycles wallet custodian.
