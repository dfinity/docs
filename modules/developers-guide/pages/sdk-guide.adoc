...
The first path is the most common: you connect to the Internet Computer running locally or on a remote network, then write, compile, and deploy in an iterative cycles. 
...
With this option, you can pre-register with the network to receive a unique identifier, then compile and deploy using a known identifier when you are ready to move your application ontp the network.