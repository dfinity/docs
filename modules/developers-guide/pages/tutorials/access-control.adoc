...
Before you can build the `+access_hello+` project, you need to connect to the {IC} network either running locally in your development environment or running on the {IC} mainnet.
...
After you connect to the {IC} network running locally in your development environment, you can register, build, and deploy your Dapp in a single step by running the link:../cli-reference/dfx-deploy{outfilesuffix}[`dfx deploy`] command.
...
After you finish experimenting with the Dapp and using identities, you can stop the local Internet Computer network so that it doesn’t continue running in the background.
