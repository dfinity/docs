...
In particular, if you are a canister user as opposed to the owner, you may want to verify that the canister is running some intended code before giving it control over your digital assets or confidential data.
Verifying this requires answering two questions:

1. Which WebAssembly (WASM) code is being executed for a canister? 
2. The canisters are rarely written in WASM directly, but normally in a higher-level language, such as Motoko or Rust.
...
To be able to answer the second questions, canister authors should ensure trusted and 
...
The impatient reader who is familiar with the topic of reproducible builds can skip straight to the <<repro-build-summary>>.
...
However, the {IC} does allow you to access the _cryptographic hash_ of the WASM code of a canister.
More concretely, it provides you the SHA-256 hash of the code.
...
We recommend using Docker, as this allows you to also fix the operating system used for building the software.
...
* To ensure that specific versions of the build tools are installed, it installs them directly, rather than through 
  `apt` (the package manager of Ubuntu, the Linux distribution running inside of the container).
...
.  You will need to ensure that any dependencies of your canister are always resolved in the same way.
   ...
   * For `npm`, running `npm install` will create a `package-lock.json` file with some fixed versions of all 
     ...
     Thus, once you are ready create the final version of your canister, run `npm install` only once.
...
.  Given the same dependencies and deterministic build scripts, the build tools themselves (`moc` for Motoko, `cargo` for Rust, `webpack` 
   ...
   Furthermore, we have observed differences between Rust code compiled to WASM under Linux and MacOS, and thus recommend fixing the platform.
...
You need to give it the build command you want to run as the first argument; here, we assume that it's just `dfx build --network ic`.
...
However, we recommend you to run the container builds under several host operating systems and comparing the results.
And finally, there are still other things to consider for the long term.
...
Thus, it'thus prudent to back up all of your toolchain and dependencies.
