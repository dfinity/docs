...
そのコマンドは以下のような出力を表示します。
+
....
("Hello, San Francisco!")
....
+
テキストを引用符で囲むことを必要とするスペースを引数が含んでいなければ、Candid インターフェース記述言語に次のようにデータタイプを推測させることができます。
+
[source,bash]
----
...
----
+
Candid はデータタイプを `Text` と推測し、プログラムの出力を以下のようなテキストとして返します。
+
....
("Hello, Paris!")
....
...
----
+
このコード例では、`+Text+` が角括弧（`+[ ]+`）で囲まれていることに気づくかもしれません。それ自体で、`+Text+` は UTF-8 の文字の集まりを表しています。
角括弧で囲まれたタイプは、そのタイプが *配列* であることを示しています。
...
dfx deploy
----
. 以下のコマンドを実行して、プログラムの中で `+location+` メソッドを呼び出し、`+city+` 引数を Candid インターフェース記述構文を使って渡します。
+
[source,bash]
----
...
----
+
そのコマンドは、Candid インターフェース記述構文にある `+(vec { val1; val2; val3; })+` を用いて、値のベクトルを返します。
...
....
. 以下のコマンドを実行して、プログラムの中で `+location_pretty+` メソッドを呼び出し、インターフェイス記述構文を使って `+city+` の引数を渡します。
+
[source,bash]
----
