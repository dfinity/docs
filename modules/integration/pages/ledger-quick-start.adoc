...
Information about other integration options and procedures will be available in future documentation.
...
A canister is similar to a "smart contract" in that it consists of the source code of a program as well as its running state, and is replicated on a block chain network that guarentees security as well as a persistant operational state.

End-users or other canisters can send messages to canister functions to perform specific operations. 
The messages can be either **query calls** that retrieve information without saving the state of the applcation or **update calls** that change and preserve the state.
...
Your account belongs to and is controlled by you, the account owner, identified by a unique IC principal. No account can be owned by two or more IC principals (i.e. no "joint accounts").
...
You can interact with the Internet Computer and ledger canister in several ways. 
...
The following summarizes the basic operational workflow for transferring ICP tokens if you're using a Rosetta node to communicate with the {IC} ledger canister. You must be an {IC} principal who authenticates to the {IC} using a signing key stored in a wallet.

After a user submits a request to the Rosetta node to make a transaction, the request is passed to the integration software running on that node to interact with the {IC} and completes the following ordered operations:
...
* Account identifiers are 32 bytes and are roughly the equivalent of the hash of a public key, possibly together with some additional sub-account specifier.
