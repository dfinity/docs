...
You create projects using the {company-id} execution command-line interface (CLI) with the command-line operations you access through the `+dfx+` executable.
...
As a best practice, you should have two terminal shells open, so that you can start and see network operation in one terminal and manage your project in another.

To start the network and deploy the `+hello+` project:

[arabic]
. Open a new terminal shell window or tab, then navigate to your project directory.
. Start the {IC} network on your local computer by running the following command in this terminal:
+
[source,bash]
----
dfx start
...
. Deploy the default program on the local network by running the following command in the second terminal:
+
[source,bash]
----
...
----
+
You should see an installation confirmation message similar to the following:
+
[source,bash]
----
