...
----
+
Because you are going to add settings to this `+canister+` section of the configuration file, you must also add a **comma** after the curly brace that encloses the location of the `+assistant+` main source code file.
. Add a new canister name and source file location for the `+rock_paper_scissors+` program and a new canister name and source file location for the `+daemon+` program files below the `+assistant+` source file location.
+
For example:
+
[source,bash,subs="quotes"]
...
[source,bash]
----
mv src/multiple_actors/ src/assistant/
----
...
----
. Deploy your `+assistant+` canister on the local network by running the following command:
+
[source,bash]
----
dfx canister install assistant
----
. Deploy the `+rock_paper_scissors+` canister on the local network by running the following command:
+
[source,bash]
----
dfx canister install rock_paper_scissors 
----
. Deploy the `+daemon+` canister on the local network by running the following command:
+
[source,bash]
----
dfx canister install daemon 
----
...
[source,bash]
----
    Dec 11 08:48:16.508 INFO Created checkpoint @2206707 in 53.769593ms
("Bob won")
----
