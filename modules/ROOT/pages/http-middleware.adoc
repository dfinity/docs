...
* Create a secure worker to contain your private key.
* Polyfill the `window.ic` to provide some mechanics to the running application so that it can communicate with the Internet Computer.
* Call the canister’s `retrieve()` method with `index.js` as the path, and evaluate it.
* Pass control of the document object model (DOM) and the page to the canister’s JavaScript code.
...
For example, in a new project, the `sample-asset.txt` file would be uploaded and available after publishing to the Internet Computer at https://CANISTER_ID.ic0.app/sample-asset.txt.
...
=== Deprecation of bootstrap and the ic0.app suffix

For a limited period of time, the {IC} will continue to support the `bootstrap` code and the use of the `ic0.app` suffix to serve canister front-ends. 
If, during this period, a request sent to a canister returns an error because the canister cannot return HTTP responses, the request will revert to using the `bootstrap` code.
At network launch, the `bootstrap` static files will be removed from `ic0.app` entirely and all projects will need to use the new architecture moving forward. In the meantime, the {sdk-short-name} team will help developers make the transition to the new architecture through the link:https://forum.dfinity.org/{DFINITY Developer Forum].
