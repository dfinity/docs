...
Therefore, there's no need to reserve a unique canister identifier on the {IC} network to hold the compiled output for the canister.

In this scenario, you can compile the canister without connecting to an {IC} network at all.
...
Before you can deploy this project, you must connect to either your local canister execution environment, provided by `dfx`, or to the Internet Computer blockchain network.
After you connect to a local canister execution environment or remote network, you must also generate a unique, *network-specific* canister identifier to replace your locally-defined identifier.
...
. In the terminal that displays network operations, press Control-C to interrupt the local canister execution environment process; or
