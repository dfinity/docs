...
Use the `+dfx wallet add-controller+` to add a controller to the wallet. An identity assigned the role of Controller has the most priviledges and can perform the flowing actions on the selected identity's cycels wallet:
