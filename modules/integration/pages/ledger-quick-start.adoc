...
The order in which update are executed is agreed upon via a consensus protocol between all IC nodes which run the canister.
...
== Integration with the IC ledger canister via the Rosetta API

It is possible to interact with the internet Computer and ledger canister in several ways. 
In this document we outline how to intergrate with the ledger canister using the the https://www.rosetta-api.org/[Rosetta application programming interface]. This is a https://www.rosetta-api.org/docs/welcome.html[well-documented and open standard] designed to support multiple block chain data formats and structured communication for exchange workflows.
...
The following steps summarize the basic operational workflow for transferring ICP tokens if using a Rosetta node to communicate with the {IC} ledger canister.  The description concerns a user who controls an IC principal ID using a signing key (stored in a wallet). 
...
In the previous sections, we outline how to integrate with the {IC} ledger canister via the Rosetta API. 
...
|`+memo+` |Allows the caller to set an optional value, e.g. to identify and link transactions for applications that communicate with the ledger canister. The Rosetta node implementation uses this field to store the nonces which ensure uniqueness of transactions. 
