...
NOTE: You can register, build, and deploy your application in a single step by running the link:../cli-reference/dfx-deploy{outfilesuffix}[`dfx deploy`] command. This tutorial demonstrates performing each of these steps independently to illustrate the difference between registering identifiers for local and network deployment and how to build and install specific canisters instead of all canisters in a project.
