...
|`+identity+` |Manages the identities used to communicate with the Internet Computer network when accessing deployed canisters.
...
You can use this command to compile all of the programs that are defined for a project in the project's `+dfx.json+` configuration file.

If you are connected to the {IC} running locally or on a sub-network, this command registers canister identifiers for all of the programs in your project and generates a `+canister_manifest.json+` file that stores details about the compiled program canisters it registered on the network.
...
You can use `+dfx identity new+` to create a new user identity named `+ic_admin+` by running the following command:

....
dfx identity new ic_admin
....

To rename an identity you have previously created, you can specify the current identity name you want to change **from** and the new name you want to change **to** by running a command similar to the following:

....
dfx identity renams test_admin ops
....
