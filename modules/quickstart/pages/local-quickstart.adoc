...
[[quick-start-intro]]
This _Quick Start_ scenario assumes that you are installing the {sdk-short-name} for the first time and want to build a canister smart contract in your *local development environment*.
...
Before you can build your first project, you need to connect to the local {IC} development environment.
As a best practice, this step requires you to have **two terminal shells** open, so that you can start and see {IC} operations in one terminal and manage your project in another.
...
. Start the local {IC} development environment on your computer in your second terminal by running the following command:
+
[source,bash]
----
dfx start
...
. Leave the terminal window that displays {IC} operations open and switch your focus to the first terminal window where you created your new project.
+
You perform the remaining steps in the terminal that doesn't display {IC} operations.

[[register-ids]]
== Register, build, and deploy the application

After you connect to the {IC} development environment, you can register, build, and deploy your dapp locally.
...
After testing the application in the browser, you can stop the local deployment so that it doesn’t continue running in the background.

To stop the local deployment:

. In the terminal that displays the development server, press Control-C to interrupt the development server process.

. In the terminal that displays {IC} operations, press Control-C to interrupt the local network process.

. Stop the local deployment running on your local computer by running the following command:
+
[source,bash]
----
dfx stop
...
* link:../developers-guide/tutorials-intro{outfilesuffix}[Tutorials] to explore building simple dapps using a local development environment.
...
* link:network-quickstart{outfilesuffix}[{IC} blockchain deployment] if you have cycles and are ready to deploy an application to the {IC} blockchain.
