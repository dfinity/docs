...
### Certified Data
To serve assets via http over `<canister-id>.ic0.app` instead of `<canister-id>.raw.ic0.app`, responses have to contain a certificate to validate their content.
...
The function `witness` is responsible for creating a tree with minimal data that still can be verified.
