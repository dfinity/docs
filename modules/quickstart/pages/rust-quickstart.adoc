= Rust Canister Developer Quick Start
:experimental:
...
This _Rust Canister Developer Quick Start_ assumes that you are installing the {cdk-short-name} for the first time. 
...
If you are comfortable meeting the prerequisites without instructions, continue to <<Download or clone the {cdk-short-name}>>.

[[download-cdk]]
== Download or clone the {cdk-short-name}
...
To download or clone from the repository:
...
. Download the {cdk-short-name} package from the `+rust_cdk+` repository by running the following command:
+
[source,bash]
----
...
+
....
rwxr-xr-x  4 pubs  staff  128 Sep 21 09:36 ic_cdk
drwxr-xr-x  4 pubs  staff  128 Sep 21 09:36 ic_cdk_macros
drwxr-xr-x  5 pubs  staff  160 Sep 21 09:36 ic_cdk_optimizer
....
