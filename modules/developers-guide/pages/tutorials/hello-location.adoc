...
To verify settings in the `+dfx.json+` configuration file:
...
If you also wanted to update the front-end, you would open the default `+src/favorite_cities/public/index.js+` file to change the canister name and make other changes. 
. Deploy your `+favorite_-_cities+` program on the local network by running the following command:
+
[source,bash]
----
