...
In particular, if you are a canister user as opposed to the owner, you likely want to verify that the canister is running some intended code before giving it control over your ICP tokens or confidential data.
...
. The canisters are normally written in a higher-level language, such as Motoko or Rust, and not directly in WASM.
   The question is then: is the WASM that's running really the result of compiling the purported source code? 
...
.  Given the same dependencies and deterministic build scripts, the build tools themselves (`moc` for Motoko, `cargo` for Rust, `webpack` 
   ...
   If you find further reproducibility issues with `moc`, we would like to hear about them.
...
It tests your build by running it in two different environments that differ in characteristics such as paths, times, file order, and others,
...
However, we recommend you to run the container builds under several host operating systems and compare the results.
