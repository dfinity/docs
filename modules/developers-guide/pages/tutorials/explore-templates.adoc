...
* The `+canisters.explore_hello+` key specifies that the main dapp to be compiled is located in the path specified by the `+main+` setting, in this case, `+src/explore_hello/main.mo+` and the `+type+` setting indicates that this is a `+motoko+` dapp.
...
Let's take a look at the sample dapp in the default `main.mo` template file as a starting point for creating simple dapp using the {proglang} programming language.

To review the default sample dapp for your project:
...
* Instead of a print function, this sample dapp defines an `+actor+` with a public `+greet+` function that takes a `+name+` argument with a type of `+Text+`.

* The dapp then uses the `+async+` keyword to indicate that the dapp returns an asynchronous message consisting of a concatenated text string constructed using `+"Hello, "+`, the `+#+` operator, the `+name+` argument, and `+"!"+`.
--
...
Before you can build the default project, you need to connect to the {IC} network either running *locally* in your development environment or running *remotely* (on-chain) on a subnet that you can access.
...
Now that you have explored the default configuration settings and dapp code and have started the {IC} network, let's compile the default dapp into an executable WebAssembly module.

To build the dapp executable:
