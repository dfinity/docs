...
To get started, let's build and deploy a simple Hello application that has just one function—called `+greet+`. The `+greet+` function accepts one text argument and returns the result with a greeting similar to **Hello,{nbsp}everyone!** in a terminal if you run the application using the command-line or in a pop-up window if you access the application in a browser.
...
If you aren’t sure how to open a new terminal shall on your local computer or how to install `node.js`, see link:newcomers{outfilesuffix}[Preliminary steps for newcomers]. 
...
You should see a path similar to the following in both terminals:
+
[source,bash]
----
/Users/<your-user-name>/ic-projects/hello
----
...
. Open a browser.
. Navigate to the default address and port number `+localhost:8000+`, then append`+?canisterId=+` and the identifier for the `+hello_assets+` canister to the URL:
+
For example, the full URL should look similar to the following:
+
[source,bash]
----
http://localhost:8000/?canisterId=ic:02000000000000000000000000000000000153
----
