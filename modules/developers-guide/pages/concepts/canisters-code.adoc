...
When you deploy the WebAssembly module that contains your program on an {IC} replica, the program is executed inside of a conceptual computational unit called a *canister smart contract*, or *canister* in short.
...
For programmers and software developers, the {IC} blockchain provides unique capabilities and opportunities within a framework that simplifies how you can design, build, and deploy applications.
