...
In addition, only the user identity that initialized the smart contract canister is assigned the `+owner+` role and only the `+owner+` and `+admin+` roles can assign roles to other users.

At a high-level, each user has a public/private key pair. The public key combined with the smart contract canister identifier the user accesses forms a security principal that can then be used as a message caller to authenticate function calls made to the smart contract canister running on the {IC}.
...
* You have stopped any {IC} processes running on the local computer.
...
Before you can build the `+access_hello+` project, you need to connect to the {IC} either running locally in your development environment or to the {IC} mainnet.

To start the {IC} locally:

. Open a new terminal window or tab on your local computer.
. Navigate to the root directory for your project, if necessary.
. Start the {IC} on your local computer by running the following command:
+
[source,bash]
----
...
----
+
After the local {IC} completes its startup operations, you can continue to the next step.

== Register, build, and deploy the dapp

After you connect to the {IC} running locally in your development environment, you can register, build, and deploy your dapp in a single step by running the link:../cli-reference/dfx-deploy{outfilesuffix}[`dfx deploy`] command.
...
+
....
Creating a wallet smart contract canister on the local network.
...
On the {IC}, a principal is the internal representative for a user, smart contract canister, node, or subnet. The textual representation for a principal is the external identifier you see displayed with working with the principal data type.
...
....
. Check the principal used to call the `+access_hello+` smart contract canister by running the following command:
+
[source,bash]
----
...
....
+
By default, the cycles wallet identifier is the principal used to call the methods in the `+access_hello+` smart contract canister.
...
After you finish experimenting with the dapp and using identities, you can stop the local {IC} so that it doesn’t continue running in the background.

To stop the local {IC}:
...
* link:../working-with-canisters{outfilesuffix}#set-owner[Set an identity to own a smart contract canister (how-to)]
