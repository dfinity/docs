...
In the link:../quickstart/rust-quickstart{outfilesuffix}[Rust Canister Developer Quick Start], you saw that creating a new project adds a default `+dfx.json+` configuration file to your project directory.
...
. Remove the `+dfx+` versioon setting.
+
...
. Check that you are still in the root directory for your project, if needed.
. Rename the default `+src/rust_counter/main.mo+` file to use the Rust file extension by running the following command:
+
[source,bash]
----
...
[dependencies]
ic_cdk = { path = "../../../rust-cdk/src/
ic_cdk", version = "0.1.0" }
