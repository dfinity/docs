...
:company-id: DFINITY
:sdk-short-name: SDK Developer Preview
...
[source,bash]
----
DFX_VERSION=0.6.8-preview sh -ci "$(curl -fsSL https://sdk.dfinity.org/install.sh)"
----
...
----
+
The command output displays information about the operations performed.
...
. Gall the `+get+` function by running the following command:
+
[source,bash]
----
...
[source,bash]
----
("Welcome to the simplified developer preview, your counter value is: 3")
----
...
|`+--all+` |Registers, builds, and deploys all of the canisters configured in the project's `+dfx.json+` file.
Note that specifying `+--all+` as a command-line option is not required. You can run `+dfx deploy+` with no arguments to deploy all canisters defined in the `+dfx.json+` file. 
