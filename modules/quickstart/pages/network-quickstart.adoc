...
For example, you might connect to the ledger using a hardware wallet, hardware security module (HSM) appliance, the Network Nervous System application, or the {sdk-short-name} `+dfx+` command-line interface. 
...
However, the principal associated with developer identity is typically not the same as your *account identifier* in the ledger.
