...
You now have a program deployed as a *canister* on your local {IC} network and can test your program by using the local URL.
...
. Add your project's `+UI canister+` ID to the URL. For example, the full URL should look similar to the following but with the `+UI canister+` identifier that was returned by dfx canister install command: 
+
[source, bash]
----
...
Review the list of function calls and types defined in the program. This program only allows a `+query+` call. 
